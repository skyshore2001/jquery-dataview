...
{var m_version='1.0';var m_exposed={getData:getData,version:function(jo){return m_version;}};var m_defaults={props:{},events:{}};function getData(jo,exact)
{if(exact)
...
{if(jo.size()==0)
return jo;var vfor;if(doInit&&(vfor=jo.attr("dv-for"))){var arrData=data[vfor];if(!$.isArray(arrData)){console.log("!!! warn: not array: "+vfor);return $([]);}
var opt1=(opt.children&&opt.children[vfor])||{};opt1.events=opt.events;var joRet=$([]);$.each(arrData,function(i,data1){var jo1=jo.clone();jo1.removeAttr("dv-for");data1.$parent=data;jo1=setDataView(jo1,data1,opt1,doInit,true);jo1.insertBefore(jo);joRet=joRet.add(jo1);});jo.remove();return joRet;}
...
var val,val1;if(doInit&&(val=jo.attr("dv-if"))){val1=!!evalWithin(data,val);if(!val1){jo.remove();return $([]);}
jo.removeAttr("dv-if");}
...
$.each(jo.children(),function(i,child){setDataView($(child),data,opt,doInit);});return jo;}
...
{if(!m_exposed[data])
$.error("*** unknown call: "+data);var args=[this];$.each(arguments,function(i,e){if(i>0)
args.push(e);});return m_exposed[data].apply(this,args);}
